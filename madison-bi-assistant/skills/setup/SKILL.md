...
1. **Check existing config** - Look for `.env` in the project root (working directory).
   If it already contains `DATABRICKS_ACCESS_TOKEN` with a real value (not a placeholder),
   skip to step 4 (verify).

2. **Collect credentials** - Ask the user for their Databricks Personal Access Token (PAT).
...
3. **Write .env** - Create or update `.env` in the project root with:
   ```
...
4. **Verify connection** - Run a test query via the MCP tool:
   ```sql
...
5. **Verify extraction tool** - Run:
   ...
   ```
   If `uv` is not installed, note this is optional (MCP queries still work) and point them
   to https://docs.astral.sh/uv/getting-started/installation/

6. **Report** - Summarise what was configured:
   ```
   ...
   - Extraction tool: [verified / skipped - uv not installed]
